...
resource "azurerm_network_interface" "this" {
  ...
  location                       = var.location
  resource_group_name            = var.resource_group_name
  accelerated_networking_enabled = each.value.index == 0 ? false : var.accelerated_networking                 # for interface 0 it is unsupported by PAN-OS
  ip_forwarding_enabled          = try(each.value.enable_ip_forwarding, each.value.index == 0 ? false : true) # for interface 0 use false per Reference Arch
  tags                           = try(each.value.tags, var.tags)
